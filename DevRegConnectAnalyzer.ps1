...
function Test-Connectivity {
    # Specify protocols we are going to use to test with to confirm TLS is not being interfered with.
    ...
    $Timeout = 500
    #itinerate through the Hashtable for each endpoint IP address to check if Port is open.
    foreach ($HashEndpoint in $IPHashtable.Keys) {
            ...
            if ($PortStatus -eq 'Open') {
                foreach ($Protocol in $ProtocolArray) {
                    ...
                    try {
                        ...
                        #If Connection fails write to the to the log the hostname, IP address, Port and protocol used.
